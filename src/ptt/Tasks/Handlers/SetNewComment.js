...
export default function SetNewComment () {
  ...
  if (this.state.frame === FrameState.otherPageofPost || this.state.frame === FrameState.firstPageofPost) {
    const forbidden = this.match(/◆ 抱歉[,，] *禁止推薦/)
    const result = forbidden || this.match(/◆ 本文已過長, 禁止快速連續推文|◆ 對不起，您的文章或推文間隔太近囉！/)
    if (result) {
      if (showAllLog)console.log('本文已過長', result)
      this.msg.PostMessage('alert', { type: 0, msg: forbidden ? 'PTT 拒絕推文：抱歉，禁止推薦。' : '推文遭暫時禁止。' })
      this.insertText('\nrG')
