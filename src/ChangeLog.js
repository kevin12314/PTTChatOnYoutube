...
export default function ChangeLog () {
  ...
  function AddChangeLogInfo () {
    ...
    changeLogInfo.v_4_5 = new Info()
    changeLogInfo.v_4_5.HoloDex.push('4.5.0：HoloDex 舊版側欄模式連線設定新增套件寬度輸入，舊版側欄模式可自行調整並立即套用。')
    changeLogInfo.v_4_5.HoloDex.push('4.5.0：HoloDex 舊版顯示模式展開 PTT 時，頂部選單不會跟著位移。')
    changeLogInfo.v_4_5.HoloDex.push('4.5.0：修正 HoloDex 定時讀取推文時，PTT 終端機搶走推文輸入框焦點，導致輸入中斷的問題。')
    changeLogInfo.v_4_5.版本.push('4.5.0：修正等待登入畫面時遺失「刪除其他重複連線」設定，並在選擇保留其他連線時提示連線上限可能導致登入斷線。')
    changeLogInfo.v_4_5.版本.push('4.5.0：修正以 AID 開啟置底文章後，多餘的同標題搜尋切換到禁止推薦的文章項目，造成手動可推文但插件無法推文的問題。')
    changeLogInfo.v_4_5.版本.push('4.5.0：推文遭 PTT 拒絕並顯示「禁止推薦」時，顯示拒絕原因並退出提示畫面，避免卡住後誤報「Ptt無回應」，保留未送出的文字。')
